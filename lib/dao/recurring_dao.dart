...
class RecurringDao {
  Future<int> create(RecurringTransaction recurring) async {
    ...
    final Map<String, dynamic> data = recurring.toJson();
    data.remove('id');
  ...
  Future<List<RecurringTransaction>> find({bool activeOnly = false}) async {
    ...
    final List<RecurringTransaction> results = [];
  ...
  Future<int> processDueTransactions() async {
    final List<RecurringTransaction> due = await findDue();
    if (due.isEmpty) return 0;

    final db = await getDBInstance();
    int processed = 0;
    for (final RecurringTransaction recurring in due) {
      ...
      final DateTime next = recurring.calculateNextDueDate();
